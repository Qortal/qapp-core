...
type UsePublishWithMetadata = {
  ...
  ) => Promise<Error | QortalGetMetadata[]>;
};
...
export function usePublish(
  ...
  const setPublishStatusByKey = usePublishStatusStore(
    (s) => s.setPublishStatusByKey
  );
  const setPublishResources = useMultiplePublishStore(
    (state) => state.setPublishResources
  );
  ...
  const fetchPublish = useCallback(
    async (metadataProp: QortalGetMetadata, disableFetch?: boolean) => {
  ...
  const deleteResource = useCallback(
    async (publish: QortalGetMetadata) => {
  ...
  const updatePublish = useCallback(
    async (publish: QortalGetMetadata, data: any) => {
      setError(null);
      setIsLoading(false);
      setHasResource(true);
      setPublish(publish, {
  ...
  const publishMultipleResources = useCallback(
    async (
      resources: ResourceToPublish[]
    ): Promise<Error | QortalGetMetadata[]> => {
