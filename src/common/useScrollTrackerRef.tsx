...
export const useScrollTrackerRef = (
  listName: string,
  hasList: boolean,
  scrollerRef: React.RefObject<HTMLElement | null> | undefined | null
) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      sessionStorage.setItem(
        SCROLL_KEY,
        scrollerRef.current.scrollTop.toString()
      );
    };
  ...
  useEffect(() => {
    if (!listName || !hasList || hasScrollRef.current || !scrollerRef?.current)
      return;
    const SCROLL_KEY = `scroll-position-${listName}`;
